...
from src.env_peg_insertion import EnvConfig, PegInsertionEnv
from src.teleop_controller import KeyboardTeleop
...
def show_controls_hud(anchor=(0.0, 0.0, 0.15), line_spacing=0.035):
    """Draw a small controls cheat-sheet in the GUI (3D text near the scene)."""
    lines = [
        "Teleop Controls",
        "Move:  X: z/c , Y: s/x ,Z: a/d ",
        "Rotate: yaw: arrow left/right, pitch: arrow up/down, roll: q/e",
        "Jaw:   'u' close, 'o' open",
        "Speed: 'k' faster, 'j' slower  |  SHIFT = fine",
    ]
    color_title = [1.0, 0.0, 0.0]
    color_text  = [1.0, 0.0, 0.0]
